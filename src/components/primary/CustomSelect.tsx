...
const CustomSelect: React.FC<CustomSelectProps> = React.memo(
    ({
        ...
        const { theme } = useApp();
        const [_options, setOptions] = useState<undefined | SelectOption[]>(
            options,
        );
        const [nextPage, setNextPage] = useState<string | null>(null);
        const [loading, setLoading] = useState<boolean>(false);
        const [fetchMoreLoading, setFetchMoreLoading] =
            useState<boolean>(false);
        const [addTagLoading, setAddTagLoading] = useState<boolean>(false);
        const [cTagReady, setCTagReady] = useState<{
            state: boolean;
            value: string | null;
        }>({
            state: false,
        ...
        const search = (
            ...
            ApiService.get(select_url, {
                ...
                .then(({ data }) => {
                    ...
                    if (data.data.items.length === 0 && mode === 'tags')
                        setCTagReady((prevState) => {
                            return {
                                state: true,
        ...
        const handlePopupScroll = (e: React.UIEvent<HTMLDivElement>) => {
            ...
            const isAtEndOfScroll =
                popupContainer.scrollTop + popupContainer.clientHeight ===
                popupContainer.scrollHeight;
        ...
        const handleAddTagClick = () => {
            if (!tag_create_url)
                throw new Error(
                    'To create a tag, please provide tag_create_url!',
                );
